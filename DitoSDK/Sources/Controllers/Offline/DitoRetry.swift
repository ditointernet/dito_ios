...
class DitoRetry {
    ...
    init(identifyOffline: DitoIdentifyOffline = .shared, trackOffline: DitoTrackOffline = .init(),
    ...
    private func checkIdentify() async -> Bool {
        guard let identify = self.identifyOffline.getIdentify,
              let id = identify.id,
              let signupRequest = identify.json?.convertToObject(type: DitoSignupRequest.self) else {
            return false
        }
    ...
    private func checkTrack() async {
        ...
        for track in tracks {
            ...
            if let reference = self.trackOffline.reference, !reference.isEmpty {
                await sendEventAsync(objectID: trackId, eventJSON: eventJSON, retry: currentRetry)
            } else {
    ...
    private func sendEventAsync(objectID: NSManagedObjectID, eventJSON: String, retry: Int16) async {
    ...
    private func checkNotification() async {
        ...
        for notification in notifications {
            ...
            if let reference = self.notificationReadOffline.reference, !reference.isEmpty, !notificationRequest.data.identifier.isEmpty {
                await withCheckedContinuation { continuation in
                    self.serviceNotification.read(notificationId: notificationRequest.data.identifier, data: notificationRequest) { [weak self] (register, error) in
