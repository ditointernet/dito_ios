...
struct DitoNotificationUnregisterDataManager {
    ...
    func save(with json: String?, retry: Int16 = 1) -> Bool {
        ...
        DitoCoreDataManager.shared.performBackgroundTask { context in
            guard let notificationUnregister = NSEntityDescription.insertNewObject(forEntityName: "NotificationUnregister", into: context) as? NotificationUnregister else {
    ...
    func update(id: NSManagedObjectID, retry: Int16) -> Bool {
        ...
        context.performAndWait {
            do {
                guard let notificationUnregister = try context.existingObject(with: id) as? NotificationUnregister else {
